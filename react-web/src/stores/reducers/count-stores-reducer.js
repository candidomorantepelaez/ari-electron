...
import { COUNT_STORES } from "stores/actions";
...
const getCountStoresState = (state) => {
  return state.stores.countStores;
}

export const getCountingClients = state => {
  return getCountStoresState(state).countingStores
};
export const getNumberOfClients = state => {
  return getCountStoresState(state).numberOfStores
};
